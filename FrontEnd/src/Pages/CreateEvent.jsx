...
const CreateEvent = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (missingField) {
      setStatus({ type: "error", message: `${missingField[1]} is required.` });
      return;
    }
    ...
    try {
      ...
      console.log("Cloudinary image URL:", response.data.event?.image);
      setUploadProgress(100);
      setSaved(true);
      setStatus({
        type: "success",
        message: `Event published successfully. Image: ${response.data.event?.image || "saved"}`,
      });
    } catch (error) {
      console.error("Event publish failed:", error.response?.data || error);
      setStatus({
        type: "error",
        message: error.response?.data?.error || "Event creation failed. Please try again.",
      });
    } finally {
      setSubmitting(false);
    }
            ...
            <div className="grid gap-5 md:grid-cols-2">
              <label className="text-sm text-gray-400">
                ...
                <input
                  ...
                  className="mt-2 w-full rounded-lg border border-white/10 bg-white/3 px-3 py-3 text-white outline-none focus:border-violet-500"
                />
              </label>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <label className="text-sm text-gray-400">
                ...
                <input
                  ...
                  onChange={(e) => updateField("maxCapacity", Number(e.target.value) || 0)}
                  className="mt-2 w-full rounded-lg border border-white/10 bg-white/3 px-3 py-3 text-white outline-none focus:border-violet-500"
                />
              </label>
            </div>
          ...
          <div className="space-y-5">
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <label className="text-sm text-gray-400">
                ...
                <input
                  ...
                  onChange={(e) => updateField("ticketPrice", Number(e.target.value) || 0)}
                  className="mt-2 w-full rounded-lg border border-white/10 bg-white/3 px-3 py-3 text-white outline-none focus:border-violet-500"
                />
              </label>
            </div>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              <label className="text-sm text-gray-400">
                ...
                <input
                  ...
                  onChange={(e) => updateField("earlyBirdPrice", Number(e.target.value) || 0)}
                  className="mt-2 w-full rounded-lg border border-white/10 bg-white/3 px-3 py-3 text-white outline-none focus:border-violet-500"
                />
              </label>
              ...
              <label className="text-sm text-gray-400">
                Featured Event
                <div className="mt-2 flex h-[52px] items-center rounded-lg border border-white/10 bg-white/3 px-3">
                  <button
                    type="button"
                    onClick={() => updateField("featured", !form.featured)}
                    className={`relative h-6 w-11 rounded-full transition ${form.featured ? 'bg-violet-600' : 'bg-gray-700'}`}
                  >
                    <span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${form.featured ? 'left-6' : 'left-1'}`} />
                  </button>
                  <span className="ml-3 text-sm text-gray-300">{form.featured ? 'Enabled' : 'Disabled'}</span>
                </div>
              </label>
            </div>
            ...
            <div className="rounded-xl border border-dashed border-violet-500/40 bg-violet-500/5 p-4 text-sm text-gray-300">
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <span className="font-semibold text-white">₹{form.ticketPrice}</span>
              </div>
              <div className="mt-2 flex items-center justify-between">
                <span>Early bird</span>
                <span className="font-semibold text-white">₹{form.earlyBirdPrice}</span>
              </div>
            </div>
          </div>
        );
        ...
        return (
          <div className="space-y-5">
            <div className="rounded-xl border border-violet-500/30 bg-violet-500/5 p-4 text-sm text-violet-100">
              ...
              <p className="mt-1">Review your content and hit publish to make the event live.</p>
            </div>
              ...
              <label className="text-sm text-gray-400">
                ...
                <div className="mt-2 flex h-[52px] items-center rounded-lg border border-white/10 bg-white/3 px-3">
                  <button
                    ...
                    className={`relative h-6 w-11 rounded-full transition ${form.live ? 'bg-emerald-500' : 'bg-gray-700'}`}
                  >
                    <span className={`absolute top-1 h-4 w-4 rounded-full bg-white transition ${form.live ? 'left-6' : 'left-1'}`} />
                  </button>
                  <span className="ml-3 text-sm text-gray-300">{form.live ? 'Live' : 'Draft'}</span>
                </div>
              </label>
            </div>
            ...
            <div className="rounded-xl border border-white/10 bg-[#10131d] p-4">
              ...
              <div className="mt-3 flex flex-col gap-3 text-sm text-gray-300">
                <div className="flex justify-between"><span>Title</span><span className="font-medium text-white">{form.title || 'Untitled Event'}</span></div>
                <div className="flex justify-between"><span>Category</span><span className="font-medium text-white">{form.category}</span></div>
                <div className="flex justify-between"><span>Date</span><span className="font-medium text-white">{form.date || 'Not set'}</span></div>
                <div className="flex justify-between"><span>Venue</span><span className="font-medium text-white">{form.venueName || 'Location not set'}</span></div>
              </div>
            </div>
          </div>
        );
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-6">
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-xs text-gray-500">
            {steps.map((step, index) => (
              <span
                key={step}
                className={`rounded-full border px-2.5 py-1 ${index === currentStep ? 'border-violet-500 bg-violet-500/10 text-violet-200' : 'border-white/10'}`}
              >
                {index + 1}. {step}
              </span>
            ))}
        ...
        <div className="grid gap-5 xl:grid-cols-[1.45fr_.75fr]">
          <section className="rounded-xl border border-white/10 bg-[#10131d] p-5 sm:p-6">
            <h3 className="mb-5 font-semibold text-white">{steps[currentStep]}</h3>
            {renderStep()}
                ...
                >
                  {submitting ? (
                    <span className="flex items-center gap-2">
                      ...
                      Publishing{uploadProgress ? ` ${uploadProgress}%` : "..."}
                    </span>
          ...
          <aside className="rounded-xl border border-white/10 bg-[#10131d] p-5">
            ...
            <div className="mt-4 overflow-hidden rounded-xl border border-white/10">
              <img src={form.imageUrl} alt={form.title || 'Event preview'} className="h-40 w-full object-cover" />
              <div className="p-4">
                ...
                <p className="mt-1 text-xs text-gray-500">
                  ◷ {form.date || "Date not set"} · {form.time || "Time not set"}
                </p>
                <div className="mt-4 flex items-center justify-between border-t border-white/10 pt-3">
                  <span className="text-xs text-gray-400">Starting from</span>
                  <span className="text-lg font-bold text-violet-400">₹{form.ticketPrice || 0}</span>
                </div>
              </div>
            </div>
            ...
            {steps.map((item, index) => (
              <div key={item} className="mt-4 flex gap-3 text-sm">
                <span className={index <= currentStep ? 'text-violet-400' : 'text-gray-600'}>◉</span>
                <div>
