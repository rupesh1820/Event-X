...
const Events = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // RANDOM ORDER
        // =========================================
        // Har page refresh par naya random order
        const randomEvents = shuffleArray(formattedEvents);

        setEve(randomEvents);

        // Refresh ke baad page 1
        setCurrentPage(1);
      } catch (error) {
        console.error(
          "Home events error:",
  ...
  const toggleFavorite = (eventId) => {
    setFavoriteEvents((currentFavorites) => {
      ...
      const updatedFavorites = currentFavorites.includes(
        normalizedId
      )
  ...
  const filteredEvents = useMemo(() => {
    ...
    // IMPORTANT:
    // Default "Upcoming First" ko random order rehne do.
    // Baaki sorting options manually sort karenge.
  ...
  // Agar filter lagane ke baad current page exist nahi karta
  useEffect(() => {
              ...
              {searchSuggestions.length > 0 ? (
                searchSuggestions.map((event) => (
                  <button
                    ...
                    className="flex w-full items-center gap-3 border-b border-gray-800 px-4 py-3 text-left hover:bg-violet-600/10"
                  >
                    <img
                      src={
                        event.image ||
